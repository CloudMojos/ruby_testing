...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match { |solution| solution.between?(0, 9)}
      end
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) { described_class.new(5, '9') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid1 = 'a'
        valid_input = '3'
        allow(game_loop).to receive(:player_input).and_return(invalid0, invalid1, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:my_game) { described_class.new(9, '9', 3)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { my_game.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:four_game) {described_class.new(2, '2', 4)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        phrase = "That was hard. It took you 4 guesses!\n"
        expect {four_game.final_message }.to output(phrase).to_stdout
      end
    end
  end
end
