...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 18 and the previous step is 25'
    subject(:random_eighteen) { described_class.new(18) }
    it 'returns 7' do
      previous_step = 25
      result = random_eighteen.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is zero' do
      subject(:random_zero) { described_class.new(0) }

      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_zero.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 1234123421' do
      subject(:random_large_int) { described_class.new(1234123421) }

      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_large_int.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is -12' do
      subject(:random_negative) { described_class.new(-12) }

      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_negative.play
        expect(result).to eq(7)
      end
    end
  end
